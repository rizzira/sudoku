...
// function preload() {
//   grid = loadJSON('puzzles/half_solved.json');
// }

// function setup() {
//   let cnv = createCanvas(2 * cellSize * 9 + 60, cellSize * 9 + 40);
//   cnv.parent('canvas-holder');
//   userGrid = createUserGrid(grid.grid);
...
// function draw() {
//   background(255);
//   // Draw solution grid on the left
//   drawGrid(grid.grid, 20, 20, false);
//   // Draw playable grid on the right
//   drawGrid(userGrid, cellSize * 9 + 40, 20, true);
//   drawFeedback(feedbackGrid, cellSize * 9 + 40, 20);
//   drawSelection();
// }
...
// function drawGrid(gridData, offsetX, offsetY, isPlayable) {
//   stroke(0);
//   for (let i = 0; i < 10; i++) {
//     let weight = (i % 3 === 0) ? 4 : 1;
//     strokeWeight(weight);
//     line(offsetX + i * cellSize, offsetY, offsetX + i * cellSize, offsetY + 9 * cellSize);
//     line(offsetX, offsetY + i * cellSize, offsetX + 9 * cellSize, offsetY + i * cellSize);
//   }
//   textSize(32);
...
//       if (num !== 0) {
//         if (isPlayable && grid.grid[row][col] !== 0) {
//           fill(180);
//         } else {
//           fill(0);
//         }
//         text(num, offsetX + col * cellSize + cellSize / 2, offsetY + row * cellSize + cellSize / 2);
//       }
//     }
//   }
// }

// function drawFeedback(feedback, offsetX, offsetY) {
//   for (let row = 0; row < 9; row++) {
//     for (let col = 0; col < 9; col++) {
...
//     strokeWeight(3);
//     noFill();
//     rect(cellSize * 9 + 40 + selectedCell.col * cellSize, 20 + selectedCell.row * cellSize, cellSize, cellSize);
//   }
// }

// function mousePressed() {
//   // Only allow selection in the playable grid (right side)
//   let offsetX = cellSize * 9 + 40;
//   let offsetY = 20;
//   if (
//     mouseX > offsetX &&
...
//     if (col >= 0 && col < 9 && row >= 0 && row < 9 && grid.grid[row][col] === 0) {
...
// // Validate according to Sudoku rules (row, col, box)
...
//     feedbackGrid[row][col] = 1; // valid move
//   } else {
//     feedbackGrid[row][col] = -1; // invalid move
//   }
// }

// let userMoves = []; // Track user moves
...
//       userGrid[selectedCell.row][selectedCell.col] = num;
//       checkEntry(selectedCell.row, selectedCell.col, num);
//       userMoves.push({ row: selectedCell.row, col: selectedCell.col }); // Track move
//       redraw();
//     }
//     if (key === 'Backspace' || key === 'Delete' || key === '0') {
//       userGrid[selectedCell.row][selectedCell.col] = 0;
...
// function draw() {
//   background(255);
//   // Draw the graph of user moves on the left
//   drawUserGraph(20, 20);
//   // Draw playable grid on the right
//   drawGrid(userGrid, cellSize * 9 + 40, 20, true);
//   drawFeedback(feedbackGrid, cellSize * 9 + 40, 20);
...
// // Draws the user's solving path as a graph
// function drawUserGraph(offsetX, offsetY) {
//   // Draw empty grid
//   drawGrid(createEmptyGrid(), offsetX, offsetY, false);
  ...
  for (let row = 0; row < 9; row++) {
    for (let col = 0; col < 9; col++) {
      ...
      if (num !== 0) {
        ...
        text(num, col * cellSize + cellSize / 2, row * cellSize + cellSize / 2);
      }
    }
  }
}
